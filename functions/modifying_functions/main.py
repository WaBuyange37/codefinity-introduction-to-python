...
Create functions to calculate the total cost of a product by applying a discount and tax, using keyword arguments and default values for flexibility.
    ...
    Define apply_discount(price, discount=0.05)
    → Returns the price after applying the discount.
    Define apply_tax(price, tax=0.07)
    → Returns the price after adding the tax.
    Define calculate_total(price, discount=0.05, tax=0.07)
    → Uses apply_discount() and apply_tax() to return the total price with both discount and tax applied.
    Call calculate_total(120) using the default discount and tax.
    Call calculate_total(100, discount=0.10, tax=0.08) using custom values via keyword arguments.
